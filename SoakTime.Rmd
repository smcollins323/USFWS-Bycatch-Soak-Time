...
```{r Load Rest of Data}
morpho.data <- read.csv("data/Main_herring_fishery_2024 - Herring_sampling.csv") %>%
  drop_na()

summaryBy(Sample_herring_wt_g ~ Port, data = morpho.data)

main.data <- read.csv("data/Main_herring_fishery_2024 - Bird_catch_BothSites.csv", na.strings = "") %>%
  ...
  left_join(metadata %>% dplyr::select(Trip_ID, Total_net_area_m2, Weather))

#Subset
sum.data <- main.data %>%
  filter(!(Trip_ID == "MH_5" & Net_group == 2)) %>%
  filter(!(Trip_ID == "MH_9" & Net_group == 2)) %>%
  group_by(Port, Trip_ID, Net_treatment, yDate) %>%
  summarise(Total_target_catch_number = sum(Total_catch_number, na.rm = T),
            Total_target_catch_kg = sum(Total_catch_wt_kg, na.rm = T),
            Net_area_m2 = sum(Total_net_area_m2)) %>%
  ungroup() %>%
...
net_mod <- glmmTMB(bycatch_sum ~ Net_treatment + yDate + Port + scaled_weight + Weather, 
...
#poisson regression
soakmod <- glmmTMB(bycatch_sum ~ Soak_duration + yDate + Port + scaled_weight,
                      data = sum.data, family = poisson(link = "log"))
...
birdmod1 <- glmmTMB(Birds ~ BirdTime + Port + yDate + Weather + scaled_weight, 
