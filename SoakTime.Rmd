...
sum.data <- main.data %>%
  ...
  mutate(bycatch_rate = bycatch_sum / Soak_duration / Net_area_m2) %>%
  mutate(scaled_weight = (Total_target_catch_kg/Net_area_m2)) %>%
  mutate(catch_rate = Total_target_catch_kg/Net_area_m2/Soak_duration) %>%
  mutate(Net_treatment = factor(Net_treatment, levels = c("24h", "12_night", "12_day"))) %>%
  mutate(Birds = ifelse(Trip_ID == "BdV_00" | Trip_ID == "BdV_16", NA, Birds)) %>%
  mutate(BycatchSpecies = ifelse(Trip_ID == "MH_15", "Birds", "Fish"))
```
