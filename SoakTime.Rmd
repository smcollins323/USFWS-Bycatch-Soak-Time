...
sum.data <- main.data %>%
  ...
  mutate(bycatch_scaled = bycatch_sum / Soak_duration / Net_area_m2) %>%
  mutate(scaled_weight = (Total_target_catch_kg/Net_area_m2)) %>%
  mutate(catch_m2h = Total_target_catch_kg/Net_area_m2/Soak_duration) %>%
...
#Clearly not normal, does contain 0s so Gamma not an option
hist(sum.data$catch_m2h)

#Get summary stats
summaryBy(catch_m2h ~ Net_treatment, data = sum.data, FUN = c(mean, median, sd))

#try model with Tweedie distribution
mod1 <- glmmTMB(catch_m2h ~ Net_treatment + Port + (1|yDate), data = sum.data,
                family = tweedie(link = "log"))
...
bycatch_scaled_line <- ggplot(data = sum.data, aes(x = Soak_duration, y = bycatch_scaled)) +
  geom_smooth(method = "loess", colour = "black") +
  scale_colour_manual(values = c("red", "blue")) +
  geom_point(aes(colour = Port)) +
  theme_classic() +
  xlab("Soak Duration (h)") +
  ylab(bquote(Bycatch~(Individuals/m^2/h))) +
  theme(legend.position = "top")

jpeg(filename = "figures/Bycatch.jpeg", width = 16, height = 18, units = "cm", res = 300)
bycatch_box / bycatch_scaled_line + plot_annotation(tag_levels = "A") + plot_layout(guides = "collect") & theme(legend.position = "bottom")
dev.off()
```
